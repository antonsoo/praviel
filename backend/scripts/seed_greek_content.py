...
async def main():
    ...
    logger.info("=" * 60)
    logger.info("CLASSICAL GREEK (PERSEUS) SEEDING - MISSING WORKS")
    logger.info("=" * 60)
    ...
    async with async_session() as session:
        ...
        # Missing Classical Greek works with TLG codes
        greek_works = [
            # (tlg_author, tlg_work, author, title, abbreviation, is_prose)
            ("tlg0020", "tlg001", "Hesiod", "Theogony", "Theog", False),
            ("tlg0020", "tlg002", "Hesiod", "Works and Days", "WD", False),
            ("tlg0011", "tlg004", "Sophocles", "Oedipus Rex", "OT", False),
            ("tlg0011", "tlg002", "Sophocles", "Antigone", "Ant", False),
            ("tlg0006", "tlg003", "Euripides", "Medea", "Med", False),
            ("tlg0016", "tlg001", "Herodotus", "Histories", "Hist", True),
            ("tlg0003", "tlg001", "Thucydides", "Peloponnesian War", "Pel", True),
        ]

        for tlg_author, tlg_work, author, title, abbr, is_prose in greek_works:
            xml_path = base_dir / tlg_author / tlg_work / f"{tlg_author}.{tlg_work}.perseus-grc2.xml"

            if xml_path.exists():
                await seed_greek_work(session, xml_path, author, title, abbr, is_prose, lang, source)
            else:
