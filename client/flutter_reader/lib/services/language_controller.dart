...
class LanguageController extends AsyncNotifier<String> {
  ...
  Future<String> build() async {
    // Mirror the synchronous selectedLanguageProvider so legacy callsites stay reactive.
    return ref.watch(selectedLanguageProvider);
  }

  /// Set the selected language by delegating to the primary provider.
  Future<void> setLanguage(String languageCode) async {
    if (!_isValidLanguageCode(languageCode)) {
      throw ArgumentError('Invalid language code: $languageCode');
    }
    await ref.read(selectedLanguageProvider.notifier).setLanguage(languageCode);
  ...
  LanguageInfo? getCurrentLanguageInfo() {
    final current = state.value ?? ref.read(selectedLanguageProvider);
    try {
      return availableLanguages.firstWhere((lang) => lang.code == current);
    } catch (_) {
      return null;
    }
  }
