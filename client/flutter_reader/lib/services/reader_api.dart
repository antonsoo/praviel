...
class ReaderApi {
  ...
  Future<TextListResponse> getTexts({String language = 'grc'}) async {
    final uri = Uri.parse(_normalize(baseUrl)).resolve('reader/texts').replace(
  ...
  Future<TextStructureResponse> getTextStructure({required int textId}) async {
    final uri = Uri.parse(_normalize(baseUrl)).resolve('reader/texts/$textId/structure');
    ...
    try {
      ...
      throw Exception('Invalid JSON from /reader/texts/$textId/structure: ${error.message}');
    } on TypeError catch (error) {
      throw Exception('Unexpected schema from /reader/texts/$textId/structure: $error');
    }
  }
  ...
  Future<TextSegmentsResponse> getTextSegments({
    ...
    final uri = Uri.parse(_normalize(baseUrl)).resolve('reader/texts/$textId/segments').replace(
      queryParameters: {
        'ref_start': refStart,
        'ref_end': refEnd,
      },
    ...
    try {
      ...
      throw Exception('Invalid JSON from /reader/texts/$textId/segments: ${error.message}');
    } on TypeError catch (error) {
      throw Exception('Unexpected schema from /reader/texts/$textId/segments: $error');
    }
  }
