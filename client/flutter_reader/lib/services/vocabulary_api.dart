...
class VocabularyApi {
  ...
  Future<VocabularyGenerationResponse> generate(
    ...
    if (response.statusCode >= 400) {
      ...
      var message = 'Vocabulary generation failed: ${response.statusCode} $reason'
          .trim();
      try {
    ...
    try {
      ...
      throw Exception('Invalid JSON from /vocabulary/generate: ${error.message}');
  ...
  Future<VocabularyInteractionResponse> recordInteraction(
    ...
    final uri = Uri.parse(_normalize(baseUrl)).resolve('vocabulary/interaction');
    final headers = <String, String>{
      'Content-Type': 'application/json',
      if (_authToken != null) 'Authorization': 'Bearer $_authToken',
    };
    ...
    if (response.statusCode >= 400) {
      final reason = response.reasonPhrase ?? '';
      var message = 'Interaction recording failed: ${response.statusCode} $reason'
          .trim();
      try {
        final payload = jsonDecode(response.body);
        if (payload is Map<String, dynamic>) {
    ...
    try {
      ...
      throw Exception('Invalid JSON from /vocabulary/interaction: ${error.message}');
    } on TypeError catch (error) {
