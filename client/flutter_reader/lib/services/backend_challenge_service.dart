...
class BackendChallengeService extends ChangeNotifier {
  BackendChallengeService(this._api, {required bool isAuthenticated})
  ...
  Future<void> load() async {
    ...
    try {
      if (!_isAuthenticated) {
        ...
        notifyListeners();
        debugPrint(
          '[BackendChallengeService] Skipping backend load for guest user (no auth token).',
        );
        return;
      }
  ...
  void _resetGuestState() {
    _dailyChallenges = [];
    _weeklyChallenges = [];
    _streak = null;
    _doubleOrNothingStatus = null;
    _userCoins = null;
    _userStreakFreezes = null;
  }
  ...
  Future<bool> updateDailyChallengeProgress({
    ...
    if (!_isAuthenticated) {
      debugPrint(
        '[BackendChallengeService] Skipping updateDailyChallengeProgress for guest user.',
      );
      return false;
    }
  ...
  Future<bool> updateWeeklyChallengeProgress({
    ...
    if (!_isAuthenticated) {
      debugPrint(
        '[BackendChallengeService] Skipping updateWeeklyChallengeProgress for guest user.',
      );
      return false;
    }
  ...
  Future<LessonCompletionResult> onLessonCompleted({
    ...
    if (!_isAuthenticated) {
      return const LessonCompletionResult(
        ...
        doubleOrNothingCompleted: false,
      );
    }
  ...
  Future<bool> purchaseStreakFreeze() async {
    if (!_isAuthenticated) {
      debugPrint(
        '[BackendChallengeService] Skipping purchaseStreakFreeze for guest user.',
      );
      return false;
    }
  ...
  Future<bool> startDoubleOrNothing({required int wager, int days = 7}) async {
    if (!_isAuthenticated) {
      debugPrint(
        '[BackendChallengeService] Skipping startDoubleOrNothing for guest user.',
      );
      return false;
    }
