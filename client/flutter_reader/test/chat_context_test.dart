...
// This test verifies the context building logic that was buggy
...
void main() {
  test('Chat context includes all previous messages (user and assistant)', () {
    ...
    expect(context.length, 2, reason: 'Context should include previous user message and assistant response');
  ...
  test('BUGGY logic (what we fixed) would exclude assistant messages with metadata', () {
    final messages = <Map<String, dynamic>>[
      {
        'role': 'user',
        'content': 'Hello',
    ...
    final buggyContext = messages
        .where((m) => m != currentUserMessage)
        .where((m) =>
          (m['translationHelp'] == null || (m['translationHelp'] as String).isEmpty) &&
    ...
    expect(
      buggyContext.length,
      1,
      reason: 'Buggy logic excluded assistant messages with translation/grammar notes',
    );
    expect(
      buggyContext[0].role,
      'user',
