...
void main() {
  ...
  testWidgets(
    'FINISH button is clickable on last lesson task',
    (WidgetTester tester) async {
      ...
      final finishButtonWidget = tester.widget<ElevatedButton>(
        find.ancestor(of: finishButton, matching: find.byType(ElevatedButton)),
      );

      expect(
        finishButtonWidget.enabled,
        isTrue,
  ...
  testWidgets(
    'FINISH button with empty answer still completes lesson',
    (WidgetTester tester) async {
      // Start the app
      app.main();
      await tester.pumpAndSettle(const Duration(seconds: 3));
