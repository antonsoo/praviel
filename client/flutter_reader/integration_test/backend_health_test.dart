...
void main() {
  ...
  group('Backend Integration Tests', () {
    test(
      'Text-range endpoint returns Greek vocabulary from specific lines',
      () async {
        ...
        final containsGreek = taskJson.codeUnits.any(
          (code) => code >= 0x0370 && code <= 0x03FF,
        );
        expect(
          containsGreek,
          isTrue,
        ...
        expect(
          foundCount,
          greaterThan(0),
          reason: 'Should contain at least one phrase from Iliad 1.20-1.30',
        );
      },
    );
    ...
    test('Register modes produce different vocabulary', () async {
      ...
      expect(
        literaryResponse.statusCode,
        200,
        reason: 'Literary register should return 200 OK',
      );
      expect(
        colloquialResponse.statusCode,
        200,
      ...
      final hasLiteraryVocab = literaryIndicators.any(
        (phrase) => literaryJson.contains(phrase),
      );
      expect(
        hasLiteraryVocab,
        isTrue,
      ...
      final colloquialIndicators = ['πωλεῖς', 'θέλω', 'οἶνον', 'φίλε'];
      final hasColloquialVocab = colloquialIndicators.any(
        (phrase) => colloquialJson.contains(phrase),
      );
      expect(
        hasColloquialVocab,
        isTrue,
